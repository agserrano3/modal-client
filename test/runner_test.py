...
def test_run_app(servicer, client) -> None:
    dummy_app = modal.App()
...
def test_run_app_unauthenticated(servicer) -> None:
    dummy_app = modal.App()
...
def test_run_app_profile_env_with_refs(servicer, client, monkeypatch) -> None:
...
def test_run_app_custom_env_with_refs(servicer, client, monkeypatch) -> None:
    monkeypatch.setenv("MODAL_ENVIRONMENT", "profile_env")
    dummy_app = modal.App()
